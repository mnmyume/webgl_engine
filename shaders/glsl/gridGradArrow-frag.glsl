...
void main() {
    ...
    // --- Arrow overlay ---
    // Local UV within the current grid cell (0..1)
    vec2 cellUV = mod(uv, gridSize) / gridSize;

    // Which cell are we in (integer index)
    vec2 cellIndex = floor(uv / gridSize);
    vec2 gradUV = (cellIndex + 0.5) / vec2(float(uNumOfGrid.x), float(uNumOfGrid.y));
    vec4 gradData = texture(uGradientTex, gradUV);
    vec2 dir = vec2(gradData.r, gradData.g);

    float dirLen = length(dir);
    if (dirLen > 0.001) {
        float angle = atan(dir.y, dir.x); // arrow.png points right (angle=0)

        // Rotate local UV around cell center (0.5, 0.5)
        ...
        vec2 arrowUV = rotated + 0.5;

        // Only sample if inside [0,1] (avoid wrapping artifacts)
        if (arrowUV.x >= 0.0 && arrowUV.x <= 1.0 && arrowUV.y >= 0.0 && arrowUV.y <= 1.0) {
            vec4 arrowSample = texture(uArrowTex, arrowUV);
            // arrow.png: orange arrow on white bg — treat dark pixels as arrow
            // Use the arrow color wherever it's not white-ish
            float arrowMask = 1.0 - smoothstep(0.6, 0.95, (arrowSample.r + arrowSample.g + arrowSample.b) / 3.0);
            gridCol = mix(gridCol, arrowSample.rgb, arrowMask * 0.85);
        }
    }
